...
final class ControllerScanner(
	...
	fun getControllers(): List<KSClassDeclaration> {
		...
		logger.info("Found ${controllerClasses.size} controllers")
	...
	private fun isValidControllerType(controllerDecl: KSClassDeclaration): Boolean {
		...
		val superTypes = controllerDecl.superTypes.map { it.resolve().declaration as? KSClassDeclaration }.filterNotNull()
